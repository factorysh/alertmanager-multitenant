...
func TestMiddleware(t *testing.T) {
	...
	assert.Equal(t, http.StatusUnauthorized, res.StatusCode, "the status code should be StatusUnauthorized")
	assert.NoError(t, err)
	// Bad JWT
	client := &http.Client{}
	...
	req, err = http.NewRequest("POST", ts.URL, bytes.NewBuffer([]byte(`{"labels": {"project": "test"}}`)))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Add("JWT", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.40OXv6OSdTvI0ySKf2LjIOQPGVo78ze2Z74T41Rq8t0")
	res, err = client.Do(req)
	assert.Equal(t, http.StatusUnauthorized, res.StatusCode, "the status code should be StatusUnauthorized")
	assert.NoError(t, err)
	// Normal Case
